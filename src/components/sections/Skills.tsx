...
import { BarChart3, Brain, Cpu, Database, FileCode, Network, Server, LineChart } from "lucide-react";
...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-24 border-b border-border-github bg-background relative">
      <div className="absolute top-1/2 right-10 w-[20rem] h-[20rem] bg-accent-blue/5 rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {skillCategories.map((category, index) => (
            <div
              key={index}
              className="p-6 bg-card-bg border border-border-github hover:border-accent-blue/40 rounded-xl transition-all duration-300 group hover:shadow-lg hover:shadow-accent-blue/5"
            >
              <div className="flex items-center gap-3.5 mb-4">
                <div className="p-3 bg-background rounded-lg border border-border-github group-hover:border-accent-blue/30 transition-colors shadow-inner">
                  {category.icon}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, sIdx) => (
                  <span
                    key={sIdx}
                    className="px-2.5 py-1 text-xs font-mono text-foreground bg-[#21262d]/40 border border-border-github hover:border-[#8b949e] hover:bg-[#21262d] rounded transition-colors"
                  >
                    {skill}
                  </span>
                ))}
        ...
        <div className="p-6 bg-card-bg border border-border-github rounded-xl">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {tools.map((tool, idx) => (
              <div
                key={idx}
                className="flex items-center gap-1.5 px-4 py-2 bg-background border border-border-github hover:border-accent-blue/60 hover:text-white rounded-lg transition-all font-mono text-sm text-text-muted shadow-sm"
              >
                <div className={`w-2 h-2 rounded-full ${
                  tool.category === "lang" ? "bg-accent-blue" :
                  tool.category === "ml" ? "bg-accent-green" :
                  tool.category === "eng" ? "bg-yellow-500" :
                  tool.category === "viz" ? "bg-red-500" :
                  tool.category === "web" ? "bg-purple-500" : "bg-text-muted"
                }`} />
                <span>{tool.name}</span>
              </div>
            ))}
